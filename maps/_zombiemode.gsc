...
// 	level.global_damage_func_ads	= maps\_zombiemode_spawner::zombie_damage_ads;
	...
	level thread print_spawn_delay();
}
...
round_think()
{
	...
	if (players.size == 1) {
		...
		calculate_delay(); // round 15 spawn rate
		level.zombie_move_speed = level.round_number * level.zombie_vars["zombie_move_speed_multiplier"]; 
		level.first_round = false; // force first round to have the proper amount of zombies
	}
	else { 
		level.round_number = 20;
		calculate_delay(); // round 20 spawn rate
		level.zombie_move_speed = level.round_number * level.zombie_vars["zombie_move_speed_multiplier"]; 
		level.first_round = false; // force first round to have the proper amount of zombies
	}
	...
	for( ;; )
	{
		level thread timer_round (level.round_number);
...
timer_round (round) {
	...
	roundTime.y += 2;
	roundTime.x -= 5;
	roundTime.foreground = true;
	roundTime.fontScale = 1.4;
	roundTime.alpha = 0;
	roundTime.color = ( 1.0, 1.0, 1.0 );
	roundTime SetTimerUp(0);
	roundTime.label = "Last Round: ";	
	...
	while ( level.round_number == round ) {
		...
		if (level.intermission) {
			roundTime.label = "Time into Round: ";
			roundTime.y += 15;
			break;
		}
	...
	displayStart = int ( getTime() / 1000 );
	
	roundTime.alpha = 1;
	while (1) {	
		roundTime setTimer ( total_time - 0.1 );
		displayEnd = int ( getTime() / 1000 );
		displayTime = displayEnd - displayStart;
		if ( displayTime >= 7) {
			break;
		}
		wait (0.5);
...
zombie_counter () {
	...
	counter.alpha = 1;
	counter.color = ( 0, 0, 0 );
	counter.label = "^1Remaining Zombies: ";

	while (1) {
		remaining = get_enemy_count() + level.zombie_total;
		counter setValue (remaining);
		wait (0.5);
	}
}

// calculate the zombie spawn delay for starting on a high round
calculate_delay() {
	flag_wait("all_players_connected");
	flag_wait("all_players_spawned");
...
print_spawn_delay() {
	...
	// spawn_delay_elem = NewHudElem();
	// spawn_delay_elem.horzAlign = "left";
	// spawn_delay_elem.vertAlign = "top";
	// spawn_delay_elem.alignX = "left";
	// spawn_delay_elem.alignY = "top";
	// spawn_delay_elem.y += 10;
	// spawn_delay_elem.x += 5;
	// spawn_delay_elem.foreground = true;
	// spawn_delay_elem.fontScale = -1.4;
	// spawn_delay_elem.alpha = 1;
	// spawn_delay_elem.label = "^1Data: ";
	// spawn_delay_elem.color = ( 0, 0, 0 );
	// spawn_delay_elem setValue(level.zombie_vars["zombie_spawn_delay"]);
}